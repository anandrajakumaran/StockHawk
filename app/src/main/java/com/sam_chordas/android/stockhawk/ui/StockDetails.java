...
import com.sam_chordas.android.stockhawk.service.StockParcelable;
...
public class StockDetails extends AppCompatActivity implements HistoricalData.HistoricalDataCallback {
    ...
    ArrayList<StockParcelable> stockParcelables = null;
...
//        setSupportActionBar(toolbar);
    ...
    @Override
    public void onSuccess(ArrayList<StockParcelable> sp) {

        this.stockParcelables = sp;

        ArrayList<Entry> entries = new ArrayList<>();
        ArrayList<String> xvalues = new ArrayList<>();

        for (int i = 0; i < this.stockParcelables.size(); i++) {

            StockParcelable stockParcelable = this.stockParcelables.get(i);
            double yValue = stockParcelable.closeValue;

            xvalues.add(Utils.convertDate(stockParcelable.date));
    ...
    public void onFailure() {
        ...
        int status = PreferenceManager.getDefaultSharedPreferences(this)
                .getInt(getString(R.string.historicalCalltatus), -1);
